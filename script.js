...
let dnsServers = loadStoredDoHServers();
...
function cloneDefaultDoHServers() {
    return DEFAULT_DNS_SERVERS.map(server => ({ ...server, ips: [...(server.ips || [])] }));
}
...
function normalizeHostnameForStorage(value) {
    ...
    const host = validateAndExtractHost(value.trim());
    return host ? host.toLowerCase() : null;
}

function loadStoredHostnames() {
    const stored = readStoredJson(SETTINGS_STORAGE_KEYS.hostnames);
    if (!Array.isArray(stored)) return cloneDefaultHostnames();

    const seen = new Set();
    const hostnames = [];
    stored.forEach(value => {
        const host = normalizeHostnameForStorage(value);
        if (!host || seen.has(host)) return;
        seen.add(host);
        hostnames.push(host);
    });

    if (stored.length > 0 && hostnames.length === 0) return cloneDefaultHostnames();
    return hostnames;
}

function normalizeDoHServerForStorage(server) {
    ...
    normalized.ips = Array.isArray(server.ips)
        ? server.ips.filter(ip => typeof ip === 'string' && ip.trim()).map(ip => ip.trim())
        : [];
    return normalized;
}

function dedupeDoHServers(servers) {
    const seenNames = new Set();
    const seenUrls = new Set();
    return servers.filter(server => {
        const nameKey = server.name.toLowerCase();
        const urlKey = server.url;
...
function loadStoredDoHServers() {
    const stored = readStoredJson(SETTINGS_STORAGE_KEYS.dohServers);
    if (!Array.isArray(stored)) return cloneDefaultDoHServers();

    const servers = dedupeDoHServers(stored.map(normalizeDoHServerForStorage).filter(Boolean));
    if (stored.length > 0 && servers.length === 0) return cloneDefaultDoHServers();
    return servers;
}

function persistHostnames() {
    writeStoredJson(SETTINGS_STORAGE_KEYS.hostnames, topWebsites);
}

function persistDoHServers() {
...
$('resetHostnames').addEventListener('click', () => {
    topWebsites = cloneDefaultHostnames();
    persistHostnames();
    renderHostsList();
    showToast('Hostnames reset to defaults', 'success');
});
...
$('addDoH').addEventListener('click', () => {
    ...
    if (dnsServers.some(s => s.url === url || s.name.toLowerCase() === name.toLowerCase())) {
...
$('resetDoHServers').addEventListener('click', () => {
    dnsServers = cloneDefaultDoHServers();
    persistDoHServers();
    renderDoHList();
    showToast('DoH servers reset to defaults', 'success');
});
...
function findResultRow(server) {
    return Array.from(resultsBody.querySelectorAll('tr[data-server-url]'))
...
function updateResult(server) {
    ...
    if (!row) {
        ...
        row.dataset.serverUrl = server.url;
    ...
    if (copyBtn) copyBtn.addEventListener('click', () => copyServerDetails(copyBtn, copyData));
...
function reliabilityBadge(reliability) {
    ...
    return `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold ${c.cls}" title="${escapeHTML(reliability.message || '')}">${c.label}${ratio}</span>`;
}
...
function colorTableRows() {
    ...
    servers.forEach(server => {
        const row = findResultRow(server);
        if (!row) return;
