...
const dnsServers = [
  ...
  },
  {
    ...
    ips: ["193.138.218.74", "194.242.2.2"],
  },
  {
    name: "NextDNS",
  ...
  },
  {
    ...
    ips: ["146.112.41.2", "146.112.41.102"],
  },
  {
    name: "DNS0.EU",
    url: "https://zero.dns0.eu/",
  ...
  },
  {
    ...
    ips: ["101.226.4.6", "180.163.224.54"],
  },
  {
  ...
  },
  {
    ...
    ips: ["94.130.180.225", "78.47.64.161"],
  },
  {
    name: "Restena",
    url: "https://kaitain.restena.lu/dns-query",
    ips: ["158.64.1.29", "158.64.1.30"],
  },
  {
    name: "IIJ",
    url: "https://public.dns.iij.jp/dns-query",
    ips: ["203.180.164.45", "203.180.166.45"],
  },
  {
    name: "LibreDNS",
    url: "https://doh.libredns.gr/dns-query",
    ips: ["116.202.176.26", "147.135.76.183"],
  },
  {
    name: "Switch",
    url: "https://dns.switch.ch/dns-query",
    ips: ["130.59.31.248", "130.59.31.251"],
  },
  {
  ...
  },
  {
    ...
    ips: ["91.239.100.100", "89.233.43.71"],
  },
];
...
function updateChartWithData(server) {
  ...
  const serverIndex = dnsChart.data.datasets.findIndex(
    (dataset) => dataset.label === server.name
  );

  // Prepare the server data for the chart (speed results for each website)
  const serverData = server.individualResults.map((result) =>
    result.speed === "Unavailable" ? null : result.speed
  );
...
async function updateLoadingMessage(message) {
  document.getElementById(
    "loadingMessage"
  ).innerHTML = `${message} <div class="spinner">
...
async function performDNSTests() {
  for (const server of dnsServers) {
    ...
    const validResults = speedResults.filter(
      (speed) => speed !== null && typeof speed === "number"
    );
    ...
    if (validResults.length > 0) {
      ...
      const median =
        validResults.length % 2 === 0
          ? (validResults[validResults.length / 2 - 1] +
...
async function measureDNSSpeed(
  dohUrl,
  hostname,
  serverType = "post",
  allowCors = false
...
function buildDNSQuery(hostname) {
  ...
  const query = new Uint8Array(
    header.length + question.length + 2 + typeAndClass.length
  );
  query.set(header);
...
function updateResult(server) {
  ...
  row.innerHTML = `
        ...
        <span class="copy-icon" onclick="copyToClipboard('DoH Server URL: ${
          server.url
        }' + '\\n' + 'IP Addresses: ${server.ips.join(
    ", "
  )}', this)">📋</span></td>
        <td class="text-center py-2 px-4 dark:text-gray-300">${
          server.speed.min !== "Unavailable"
            ? server.speed.min.toFixed(2)
            : "Unavailable"
        }</td>
        <td class="text-center py-2 px-4 dark:text-gray-300">${
          server.speed.median !== "Unavailable"
            ? server.speed.median.toFixed(2)
            : "Unavailable"
        }</td>
        <td class="text-center py-2 px-4 dark:text-gray-300">${
          server.speed.max !== "Unavailable"
            ? server.speed.max.toFixed(2)
            : "Unavailable"
        }</td>
    `;
  ...
  detailsRow.innerHTML = `
    <td colspan="4" class="py-2 px-4 dark:bg-gray-800 dark:text-gray-300">
        ...
        <ul>
            ${server.individualResults
              .map((result) => {
                if (typeof result.speed === "number") {
                  return `<li>${result.website}: ${result.speed.toFixed(
                    2
                  )} ms</li>`;
                } else {
  ...
  rowPairs.sort((a, b) => {
    ...
    if (!cellA || !cellB) {
      console.error("Undefined cell encountered", {
        cellA,
        cellB,
        rowIndexA: a[0].rowIndex,
        rowIndexB: b[0].rowIndex,
      });
      return 0;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderList() {
    ...
    topWebsites.forEach((site, index) => {
      ...
      li.className =
        "px-2 py-1 mb-1 bg-gray-200 rounded flex justify-between items-center border-b border-gray-300 dark:bg-gray-700 dark:border-gray-600";
      ...
      removeBtn.className =
        "bg-red-500 text-white rounded px-2 py-1 hover:bg-red-600 dark:bg-red-700 dark:hover:bg-red-800";
